...
interface SidebarProps {
  ...
  onChangePassword: () => void; // New prop for showing change password modal
}
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  }, [selectedKey, menuItems, categories, openCategories]);
  ...
  return (
    <aside className="w-72 bg-white text-gray-700 p-4 space-y-1 flex flex-col overflow-y-auto border-r border-gray-200 flex-shrink-0">
      <div 
        className="text-2xl font-semibold text-center text-sky-600 py-3 mb-4 cursor-pointer hover:text-sky-700 transition-colors"
        onClick={() => onSelectNavigation('dashboard')}
        role="button"
        tabIndex={0}
      ...
      <nav className="flex-grow space-y-1">
        ...
        <div>
          <button
            onClick={() => onSelectNavigation('planning', 'planning')}
